...
from vista.dataset.path_config import SCAN_FAMILY_BASE
from vista.model.vision.basic_modules import (_get_clones, calc_pairwise_locs,
                                        get_mlp_head, init_weights, get_mixup_function)
from vista.model.vision.pointnet2.pointnet2_modules import PointnetSAModule
from vista.model.vision.transformers import (TransformerDecoderLayer,
...
class PointTokenizeEncoder(nn.Module):
    ...
    def forward(self, obj_pcds, obj_locs, obj_masks, obj_sem_masks, obj_labels=None, cur_step=None, max_steps=None):
        ...
        if self.mixup_strategy != None:
            obj_sem_cls_mix = self.mixup_function(obj_sem_cls, obj_labels, cur_step, max_steps) # With some probability, replaces predicted class labels by ground truth labels
        else:
